...
def getLatexTable(TP, FN, FP, TN, TPR, SPC, PPV, NPV, FPR, FDR, FNR, ACC,table_name='table_1'):
    float_formatter = lambda x: "%.2f" %( x*100)
    TPR=TPR*100
    #, SPC, PPV, NPV, FPR, FDR, FNR, ACC
    fileOut=open('%s.txt'%(table_name),'w')
...
def main(parameters, samples, labels):
    ...
    cFea=0
    cErr=0
    confSVM=confFEAR=confMLP =confMIX= confBOOST=confSVM2=confMLP2=numpy.matrix(numpy.zeros((defaults.CLASS_N, defaults.CLASS_N), numpy.int32))
    for _ in xrange(100):
        ...
        print resMLP != labels_test
        print numpy.mean(resMLP != labels_test)
        ...
        print "SVM2"
        confSVM2=confSVM2+numpy.matrix(modelSVM.evaluate(None, labels_test, resSVM2))
        print "MLP2"
        confMLP2=confMLP2+numpy.matrix(modelSVM.evaluate(None, labels_test, resMLP2))
    ...
    print "TP_svm2:"
    TP_svm2=numpy.diagonal(confSVM2)
    print TP_svm2
    print
    
    print "FN_svm2:"
    FN_svm2=(confSVM2.sum(axis=1).transpose()-TP_svm2)[0]
    print FN_svm2
    print
    
    print "FP_svm2"
    FP_svm2=confSVM2.sum(axis=0)-TP_svm2
    print FP_svm2
    print

    print "TN_svm2"
    TN_svm2=confSVM2.sum()*numpy.ones((1, defaults.CLASS_N), numpy.int32)-FP_svm2-FN_svm2-TP_svm2
    print TN_svm2
    print
    
    print "TPR or Sensitivity: TPR=TP/(TP+FN)"
    TPR_svm2=numpy.float32(TP_svm2)/(TP_svm2+FN_svm2)
    print TPR_svm2
    print
    
    print "Specificity (SPC) or true negative rate (TNR): TPR=TN/(FP+TN)"
    SPC_svm2=numpy.float32(TN_svm2)/(TN_svm2+FP_svm2)
    print SPC_svm2
    print
    
    print "precision or positive predictive value (PPV): PPV=TP/(TP+FP))"
    PPV_svm2=numpy.float32(TP_svm2)/(TP_svm2+FP_svm2)
    print PPV_svm2
    print
    
    print "negative predictive value (NPV): NPV=TN/(TN+FN))"
    NPV_svm2=numpy.float32(TN_svm2)/(TN_svm2+FN_svm2)
    print NPV_svm2
    print
    ...
    print "miss rate or false negative rate (FNR) FNR=FN/(FN+TP)"
    FNR_svm2=numpy.float32(FN_svm2)/(FN_svm2+TP_svm2)
    print FNR_svm2
    print
    
    print "accuracy (ACC) ACC=TP+TN/(TP+TN+FP+FN)"
    ACC_svm2=numpy.float32(TP_svm2+TN_svm2)/(FN_svm2+TP_svm2+FP_svm2+TN_svm2)
    print ACC_svm2
    print
    
    test_rateSVM2=100-test_rateSVM2/100
    print "test_rateSVM2", test_rateSVM2
    ...
    print "TP:"
    TP_mlp2=numpy.diagonal(confMLP2)
    print TP_mlp2
    print
    
    print "FN:"
    FN_mlp2=(confMLP2.sum(axis=1).transpose()-TP_mlp2)[0]
    print FN_mlp2
    print
    
    print "FP"
    FP_mlp2=confMLP2.sum(axis=0)-TP_mlp2
    print FP_mlp2
    print

    print "TN"
    TN_mlp2=confMLP2.sum()*numpy.ones((1, defaults.CLASS_N), numpy.int32)-FP_mlp2-FN_mlp2-TP_mlp2
    print TN_mlp2
    print
    
    print "TPR or Sensitivity: TPR=TP/(TP+FN)"
    TPR_mlp2=numpy.float32(TP_mlp2)/(TP_mlp2+FN_mlp2)
    print TPR_mlp2
    print
    
    print "Specificity (SPC) or true negative rate (TNR): TPR=TN/(FP+TN)"
    SPC_mlp2=numpy.float32(TN_mlp2)/(TN_mlp2+FP_mlp2)
    print SPC_mlp2
    print
    
    print "precision or positive predictive value (PPV): PPV=TP/(TP+FP))"
    PPV_mlp2=numpy.float32(TP_mlp2)/(TP_mlp2+FP_mlp2)
    print PPV_mlp2
    print
    
    print "negative predictive value (NPV): NPV=TN/(TN+FN))"
    NPV_mlp2=numpy.float32(TN_mlp2)/(TN_mlp2+FN_mlp2)
    print NPV_mlp2
    print
    ...
    print "miss rate or false negative rate (FNR) FNR=FN/(FN+TP)"
    FNR_mlp2=numpy.float32(FN_mlp2)/(FN_mlp2+TP_mlp2)
    print FNR_mlp2
    print
    
    print "accuracy (ACC) ACC=TP+TN/(TP+TN+FP+FN)"
    ACC_mlp2=numpy.float32(TP_mlp2+TN_mlp2)/(FN_mlp2+TP_mlp2+FP_mlp2+TN_mlp2)
    print ACC_mlp2
    print
    
    test_rateMLP2=100-test_rateMLP2/100
    print "test_rateMLP2", test_rateMLP2
    ...
    getLatexTable([TP_svm,TP_mlp,TP_MIX],[FN_svm,FN_mlp,FN_MIX], [FP_svm,FP_mlp,FP_MIX], [TN_svm,TN_mlp,TN_MIX], [TPR_svm,TPR_mlp,TPR_MIX], [SPC_svm,SPC_mlp,SPC_MIX], [PPV_svm,PPV_mlp,PPV_MIX],
                  [NPV_svm,NPV_mlp,NPV_MIX], [FPR_svm,FPR_mlp,FPR_MIX], [FDR_svm,FDR_mlp,FDR_MIX], [FNR_svm,FNR_mlp,FNR_MIX], [ACC_svm,ACC_mlp,ACC_MIX],table_name='table_1')
    
    getLatexTable([TP_svm2,TP_mlp2,TP_MIX],[FN_svm2,FN_mlp2,FN_MIX], [FP_svm2,FP_mlp2,FP_MIX], [TN_svm2,TN_mlp2,TN_MIX], [TPR_svm2,TPR_mlp2,TPR_MIX], [SPC_svm2,SPC_mlp2,SPC_MIX], [PPV_svm2,PPV_mlp2,PPV_MIX],
                  [NPV_svm2,NPV_mlp2,NPV_MIX], [FPR_svm2,FPR_mlp2,FPR_MIX], [FDR_svm2,FDR_mlp2,FDR_MIX], [FNR_svm2,FNR_mlp2,FNR_MIX], [ACC_svm2,ACC_mlp2,ACC_MIX],table_name='table_2')
